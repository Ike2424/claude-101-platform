...
// ============================================================
// IMPORTANTE: la inicialización de la DB ya NO bloquea la carga del módulo.
// Antes, si DATABASE_URL faltaba o Postgres no respondía, `throw` en
// top-level await tumbaba TODO server.js → Railway veía "Healthcheck failure"
// sin pista. Ahora capturamos errores y los exponemos via isReady()/lastError;
// el server arranca, /healthz responde, y /api/status muestra el problema.
// ============================================================
...
try {
  if (DRIVER === 'sqlite') {
    ...
    _db = await connectWithRetry(() => {
      ...
      return inst;
    }, `sqlite (${dbPath})`, 3); // menos reintentos: si falla, falla rápido
    _ready = true;
    ...
    // Test de conexión en background — NO bloqueamos el module load.
    // Si Postgres no responde al arrancar, /healthz seguirá OK y /api/status
    // mostrará el error. El operador puede reconfigurar sin bucle de reinicios.
    connectWithRetry(async () => {
      ...
      .then(() => { _ready = true; })
      .catch(err => { lastError = err; logger.error({ err }, 'Postgres no disponible — operando degradado'); });
  } else {
    throw new Error(`DB_DRIVER desconocido: ${DRIVER}`);
  }
} catch (err) {
  lastError = err;
  logger.error({ err, driver: DRIVER }, 'Init de DB falló — el servidor arranca igualmente para diagnóstico');
}
