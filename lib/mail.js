...
import { signUnsub } from './token.js';
...
// URL de baja de un lead (firma el id, nunca el email).
export function unsubUrl(leadId) {
  const base = process.env.PUBLIC_URL || 'https://academia101.com';
  return `${base}/libro/baja?token=${encodeURIComponent(signUnsub(leadId))}`;
}

// Pie de baja obligatorio en todo email de marketing (HTML + texto).
function unsubFooter(url) {
  return {
    html: `<hr style="border:0;border-top:1px solid #E1D8C2;margin:24px 0;"><p style="color:#A39782;font-size:11px;line-height:1.55;margin:0;">Recibes este email porque te suscribiste en academia101.com. Si no quieres seguir recibiéndolos, <a href="${url}" style="color:#A39782;text-decoration:underline;">date de baja aquí</a> (un solo clic).</p>`,
    text: `\n\n—\nRecibes este email porque te suscribiste en academia101.com.\nDarte de baja (un solo clic): ${url}`,
  };
}

// Despachador genérico (resend / smtp / console).
async function dispatch({ to, subject, html, text }) {
  if (PROVIDER === 'console') {
    logger.info(`\n──── ✉️  MARKETING (console) ──── Para: ${to} · Asunto: ${subject}\n`);
...
// Envío de marketing: SIEMPRE lleva pie de baja ligado al lead.
export async function sendMarketingEmail({ to, leadId, subject, html, text }) {
  if (leadId == null) throw new Error('sendMarketingEmail requiere leadId (para el enlace de baja)');
  const f = unsubFooter(unsubUrl(leadId));
  return dispatch({ to, subject, html: (html || '') + f.html, text: (text || '') + f.text });
}
...
<html><body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f5f5f5; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 8px; padding: 32px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    ...
    <p style="font-size: 16px; line-height: 1.6; color: #333; margin: 0 0 24px;">
      ¡Felicidades! Tu acceso está <strong>LISTO ahora mismo</strong>.
    </p>
    <p style="font-size: 14px; line-height: 1.6; color: #c8542b; margin: 0 0 24px; font-weight: 600;">
      ⚡ Dato importante: 80% de los que compran pero NO empiezan en 48h nunca termina el curso. Así que: dedica 30 minutos HOY al módulo 1. Verás resultados inmediatamente.
    </p>
...
// Email de recuperación de carrito abandonado (checkout iniciado pero no completado)
export async function sendCartRecoveryEmail({ to, recoveryUrl }) {
  const url = recoveryUrl || `${process.env.PUBLIC_URL || 'https://academia101.com'}/`;
  const subject = 'Te quedaste a un paso de Claude 101 👀';
  const text = `Hola,\n\nEmpezaste tu compra de Claude 101 pero no llegaste a terminarla. Tu carrito sigue guardado y puedes completar el pago en 1 minuto aquí:\n\n${url}\n\nSi tuviste algún problema o alguna duda, responde a este email y te ayudamos.\n\nUn saludo,\nClaude 101`;
  const html = `
<!doctype html>
<html><body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; background: #F4EFE3; padding: 32px;">
  <div style="max-width: 480px; margin: 0 auto; background: #FBF8F0; border: 1px solid #E1D8C2; border-radius: 16px; padding: 36px;">
    <h1 style="font-family: Georgia, serif; color: #1C1A16; font-size: 26px; margin: 0 0 12px;">Te quedaste a un paso de <em style="color: #C8542B;">Claude 101</em></h1>
    <p style="color: #3A352D; line-height: 1.6; font-size: 15px; margin: 0 0 22px;">Empezaste tu compra pero no llegaste a terminarla. Tu carrito sigue guardado — puedes completar el pago en menos de un minuto.</p>
    <p style="margin: 0 0 24px;">
      <a href="${url}" style="display:inline-block; background: #1C1A16; color: #F4EFE3; padding: 14px 24px; border-radius: 100px; text-decoration: none; font-weight: 600; font-size: 14px;">Completar mi compra →</a>
    </p>
    <p style="color: #3A352D; line-height: 1.6; font-size: 14px; margin: 0 0 8px;">Recuerda lo que incluye:</p>
    <ul style="color: #3A352D; font-size: 14px; line-height: 1.6; margin: 0 0 22px; padding-left: 20px;">
      <li>9 módulos · acceso vitalicio</li>
      <li>Actualizaciones incluidas</li>
      <li>Garantía de reembolso</li>
    </ul>
    <p style="color: #756B5B; font-size: 12px; line-height: 1.55; margin: 0;">¿Tuviste algún problema con el pago o alguna duda? Responde a este email y te ayudamos personalmente.</p>
    <hr style="border: 0; border-top: 1px solid #E1D8C2; margin: 28px 0;">
    <p style="color: #A39782; font-size: 11px; margin: 0;">Si ya completaste tu compra o no quieres terminarla, ignora este mensaje.</p>
  </div>
</body></html>`;

  if (PROVIDER === 'console') {
    logger.info('\n──────── ✉️  CART RECOVERY (modo console) ────────');
    logger.info(`Para: ${to}`);
