...
// Token de baja de marketing: firma SOLO el id del lead (nunca el email),
// sin caducidad (un enlace de baja debe funcionar siempre). Purpose acotado
// para que no valga como sesión.
export function signUnsub(leadId) {
  return jwt.sign({ lid: leadId, purpose: 'unsub' }, SECRET);
}

export function verifyUnsub(token) {
  try {
    const p = jwt.verify(token, SECRET);
    return p && p.purpose === 'unsub' && p.lid != null ? p : null;
  } catch {
    return null;
  }
}
